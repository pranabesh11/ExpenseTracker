...
const Settings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        setForm(initialForm);
        const profile = rows.find((item: any) => item.key === "profileImage");
        const qr = rows.find((item: any) => item.key === "upiQrCode");
        if (profile) {
          setProfileImage("http://localhost:8080" + profile.value);
        }
        if (qr) {
          setQrImage("http://localhost:8080" + qr.value);
        }
      } catch (e) {
        console.warn(e);
      }
  ...
  return (
    <div className="settingsPage">
      <div className="settingsCard">
        ...
        <div className="avatarSection">
          ...
          <div className="avatarActions">
            <Button icon={<EyeOutlined />} onClick={() => setShowImg(true)}>
              View
            </Button>
            ...
            >
              <Button icon={<CameraOutlined />}>Update</Button>
            </Upload>
        ...
        <div className="settingsGrid">
          {fields
            .filter(
              (field) =>
                field.key !== "profileImage" && field.key !== "upiQrCode",
            )
            .map((field) => (
              <div
                className={
                  field.type === "TEXTAREA" ? "field fullWidth" : "field"
                }
                key={field.key}
              >
      ...
      >
        {profileImage ? (
          <Image width="100%" alt="Profile" src={profileImage} />
        ) : (
          <div style={{ textAlign: "center" }}>No profile image selected</div>
        )}
      </Modal>
    </div>
  );
